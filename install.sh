...
Установка и обновление Watchgoose.
  ...
  bash install.sh           установка: спросит домен, бюджет, S3
  bash install.sh --update  обновление: ничего не спрашивает, берёт всё из .env
  bash install.sh --update --tag=1.1.0
                            обновление до конкретной версии

  ⚠️ Запускать через bash, а не ./install.sh: файл приезжает по curl,
     а curl прав на запуск не ставит. Здесь стояло ./install.sh — и первое,
     что видел человек после выкладки, было «Permission denied».
