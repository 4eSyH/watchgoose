...
#
# Поэтому он печатается открыто и последним, вместе с командами, чтобы
# посмотреть его снова. Тот же приём, что и с токеном: показать в момент,
# когда человек ещё смотрит в терминал.
...
if [ -n "$ARCHIVE_KEY" ]; then
    ...
    warn "потеряете — не открыть ни архив в S3, ни копии базы. Восстановить нечем."
